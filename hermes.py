...
@circuit_breaker(lambda input_text: "DeepSeek‑R1 API request failed; using internal logic.")
...
def get_llm():
    ...
    if settings.local_only_llm:
        ...
        try:
            test_response = ollama.chat(model="mistral", messages=[{"role": "user", "content": test_message}])
        ...
        try:
            test_response = ollama.chat(model="deepseek-r1", messages=[{"role": "user", "content": test_message}])
            if "message" in test_response and "content" in test_response["message"]:
...
class RLAgent:
    ...
    def update(self, feedback: Dict[str, Any]):
        self.logic_weight = max(0.3, min(0.7, self.logic_weight + random.uniform(-0.05, 0.05)))
...
class Apre:
    def process(self, session_id: str, input_data: str, past_convo: List[Tuple[str, Any]]) -> str:
        with tracer.start_as_current_span("Apre.process"):
            ...
            if any(x in result.lower() for x in ["no useful response", "offline", "failed"]):
                try:
...
class Magi:
    ...
    def process(self, session_id: str, input_data: str, past_convo: List[Tuple[str, Any]]) -> str:
        with tracer.start_as_current_span("Magi.process"):
            ...
            if numbers:
                try:
                    numerology_insight = f"Numerology Insight: {sum(numbers)} holds deep significance."
                except Exception as e:
            ...
            optimization_suggestion = past_feedback[0] if isinstance(past_feedback, list) and past_feedback else "New optimization strategy."
...
@ray.remote
class Chochmah:
    def process(self, session_id: str, input_data: str, depth: int, breadth: int) -> str:
        with tracer.start_as_current_span("Chochmah.process"):
            logger.info(f"[Chochmah] Starting deep research for session {session_id} with depth {depth} and breadth {breadth}.")
...
class Tiferet:
    ...
    def process(self, session_id: str, input_data: str) -> str:
        with tracer.start_as_current_span("Tiferet.process"):
            try:
                logic_response = ray.get(self.apre.process.remote(session_id, input_data, []),
                                         timeout=settings.timeout_seconds)
                creative_response = ray.get(self.magi.process.remote(session_id, input_data, []),
                                            timeout=settings.timeout_seconds)
            except Exception as e:
            ...
            return self._harmonize_output(logic_response, creative_response, balance_factor)

    def _calculate_harmony_weight(self, input_text: str) -> float:
        words = input_text.split()
        word_count = len(words)
        complexity_factor = min(word_count / 50.0, 1.0)
        dynamic_weight = self.default_weight * (1 - complexity_factor) + complexity_factor * 0.75
...
class Keter:
    ...
    def orchestrate(self, session_id: str, input_data: str, depth: int = None, breadth: int = None) -> Dict[str, Any]:
        with tracer.start_as_current_span("Keter.orchestrate"):
            ...
            try:
                research_result = ray.get(
                    self.modules["Chochmah"].process.remote(session_id, input_data, depth, breadth),
...
async def process_input(session_id: str, input_text: str) -> Dict[str, Any]:
    with tracer.start_as_current_span("process_input"):
        ...
        try:
            ...
            response_future = loop.run_in_executor(None, ray.get, keter.orchestrate.remote(session_id, input_text))
            response = await asyncio.wait_for(response_future, timeout=settings.timeout_seconds)
        except Exception as e:
...
async def deep_research(session_id: str, input_text: str, depth: int = None, breadth: int = None) -> Dict[str, Any]:
    with tracer.start_as_current_span("deep_research"):
        ...
        try:
            loop = asyncio.get_running_loop()
            report_future = loop.run_in_executor(None, ray.get, chochmah_actor.process.remote(session_id, input_text, depth, breadth))
            report = await asyncio.wait_for(report_future, timeout=(settings.timeout_seconds * (depth + 1)))
        except Exception as e:
