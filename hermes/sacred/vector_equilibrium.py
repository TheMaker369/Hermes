...
class VectorEquilibrium:
    ...
    def calculate_torsion_field(self, points: np.ndarray) -> np.ndarray:
        ...
        for idx in np.ndindex(points.shape[:-1]):
            ...
            curl_x = (self.get_force_field(point + np.array([0, h, 0]))[2] -
                     self.get_force_field(point - np.array([0, h, 0]))[2]) / (2*h)
            curl_y = (self.get_force_field(point + np.array([0, 0, h]))[0] -
                     self.get_force_field(point - np.array([0, 0, h]))[0]) / (2*h)
            curl_z = (self.get_force_field(point + np.array([h, 0, 0]))[1] -
                     self.get_force_field(point - np.array([h, 0, 0]))[1]) / (2*h)
